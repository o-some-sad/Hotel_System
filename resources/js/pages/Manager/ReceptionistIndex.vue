...
// Modal controls
const openFormModal = (receptionist = null) => {
  form.clearErrors()
  form.reset()
...
const submitForm = () => {
  ...
  if (formMode.value === 'create') {
    router.post('/receptionists', formData, options)
  } else {
    router.post(`/receptionists/${form.id}`, formData, options)
  }
}

const deleteReceptionist = receptionist => {
  if (confirm(`Are you sure you want to delete this receptionist?\nID: ${receptionist.id}\nName: ${receptionist.name}\nEmail: ${receptionist.email}`)) {
    router.delete(`/receptionists/${receptionist.id}`, {
      ...
      onError: (error) => {
        alert(error.response.data.message || 'An error occurred while deleting the receptionist.')
      }
    })
  }
}
        ...
        <TableRow v-for="mgr in receptionists.data" :key="mgr.id" class="hover:bg-muted/50">
          ...
          <TableCell>
            <div class="flex space-x-2">
              <Button 
                @click="openFormModal(mgr)" 
                v-if="isAdmin || ids?.includes(mgr.id)" 
                size="sm" 
                variant="outline"
              >
                Edit
