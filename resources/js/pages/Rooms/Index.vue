<template>
    <AppLayout :title="'Manage Rooms'" :breadcrumbs="breadcrumbs">
        <div class="container py-6">
                        ...
                        <TableRow v-for="room in filteredRooms" :key="room.id">
                            ...
                            <TableCell class="text-right">
                                <div class="flex justify-end gap-2">
                                    ...
                                    <Button v-if="canManage(room)" variant="ghost" size="icon"
            ...
            <div v-if="rooms.links && rooms.links.length > 3" class="mt-6 flex justify-center">
                <div class="flex items-center gap-1">
                    ...
                    <Button :disabled="!rooms.prev_page_url" @click="changePage(rooms.current_page - 1)"
                    ...
                    <div class="flex">
                        <div v-for="(link, i) in rooms.links" :key="i">
                            ...
                            <div v-if="i > 0 && i < rooms.links.length - 1">
                                <Button :variant="link.active ? 'default' : 'outline'" @click="changePage(link.label)"
                                    class="mx-1">
                                    {{ link.label }}
                                </Button>
                            </div>
                    ...
                    <Button :disabled="!rooms.next_page_url" @click="changePage(rooms.current_page + 1)"
                        variant="outline" size="icon">
        ...
        <!-- Edit Room Dialog -->
        <EditRoomDialog v-if="selectedRoom" v-model:show="showEditDialog" :room="selectedRoom" :floors="floors"
            :is-admin="isAdmin" @updated="fetchRooms" />

        <!-- Delete Confirmation Dialog -->
        <DeleteConfirmationDialog v-if="selectedRoom" v-model:show="showDeleteDialog" :room="selectedRoom"
            @deleted="fetchRooms" />
    </AppLayout>
</template>
